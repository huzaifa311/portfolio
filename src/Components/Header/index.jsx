...
const Header = ({ isDarkTheme, custom1, custom2, custom3, custom4, custom5, onClick }) => {
    ...
    return (
        <div className={`shadow-2xl w-full fixed top-0 left-0 z-[100] ${custom1}`}>
            <div className="md:flex items-center justify-between wrapper py-4 md:px-10 px-7">
                ...
                <ul
                    className={`animate__animated animate__backInLeft md:flex md:items-center absolute md:static text-white wrapper md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? "top-12" : "top-[-490px]"
                        }`}
                >
